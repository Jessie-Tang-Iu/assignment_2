...
public class ManageComputers {
    ...
    private static void addComputer(ArrayList<Object> computers, Scanner s) {
        ...
        switch (computerType) {
            ...
            case "l":
                ...
                String[] validScreens = { "13", "14" };
                String screenSize = getValidatedInput(s, "Enter screen size:", validScreens);
            ...
            case "d":
                ...
                String GPUType = getValidatedInput(s, "Enter GPU:", validGPU);
    ...
    private static Computer getComputerData(Scanner s) {
        String[] validCPUs = { "i5", "i7" };
        String[] validRAM = { "16", "32" };
        String[] validDisk = { "512", "1024" };

        String CPU = getValidatedInput(s, "Enter CPU:", validCPUs);
        String RAM = getValidatedInput(s, "Enter RAM:", validRAM);
        String disk = getValidatedInput(s, "Enter Disk:", validDisk);
        ...
        while (!valid) {
            ...
            for (String allowed : whitelist) {
                if (input.equalsIgnoreCase(allowed)) {
                    valid = true;
                    input = allowed; // normalize case
                    break;
                }
            }

            if (!valid) {
                System.out.println("Invalid input. Allowed values: " + String.join(", ", whitelist));
            }
        }
        return input;
    }
